...
| `memoring import` | `[provider]` / `--file` `--text` stdin / `--default-sensitivity` / `--dry-run` / `list` / `promote <id> --scope <l> [--sensitivity]` / `reject <id>` / `--print-prompt <provider>` | Imports a pasted foreign-AI export (ChatGPT/Claude/Gemini) into the active Realm as non-authoritative candidates: each entry is captured raw-first, normalized to a `host_memory` Event (never independent evidence) and a `candidate` Claim with no evidence, and is **never auto-consolidated** (`consolidatePending` skips it). Unparseable input is quarantined (no raw loss). `promote` is the explicit user authority (§5.2) that confirms a candidate as a recallable `created_by:user` memory; `reject` drops it. secret-scan runs on every entry. See ADR-0007. |
